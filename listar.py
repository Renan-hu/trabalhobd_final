...
#FUNÇÃO QUE SERVE PARA PODER CHAMAR NA INTERFACE
...
def listar_func() :
    ...
    def listar_clientes():
        ...
        cursor.execute(
            ...
            "clientes.endereco, clientes.localidade, clientes.data_nascimento, clientes.status, "
            "GROUP_CONCAT(cliente_telefones.numero SEPARATOR ', ') AS telefones "
            "FROM clientes "
            "LEFT JOIN cliente_telefones ON clientes.id = cliente_telefones.cliente_id "
    ...
    def deleta_cliente():
        ...
        if messagebox.askyesno("Confirmar", "Deseja realmente deletar este cliente?"): #abre uma box que tem as opção sim ou n (.aksyesno)
            ...
            cursor.execute("DELETE FROM cliente_telefones WHERE cliente_id = %s", (cliente[0],))
